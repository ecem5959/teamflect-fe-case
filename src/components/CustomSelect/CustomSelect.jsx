            ...
            <div className={`select ${isOpen ? 'open' : ''}`} onClick={toggleDropdown}>
                        ...
                        <div className="selected">
                            {
                                showImage && selectedOption.avatar && (
                                    <img src={selectedOption.avatar} alt={selectedOption.name} className="selectedImg" width={24} height={24}/>
                                )
                            }
                ...
                {isOpen && (
                    <ul className="optionList">
                        {options.map(option => (
                            <li key={option.id} className="optionListItem" onClick={() => handleSelect(option)}>
                                {
                                    showImage && option.avatar && (
                                        <img src={option.avatar} alt={option.name} className="optionImg" width={24} height={24}/>
                                    )
                                }
